# create tuples with either aligned variables (positive) or no alignment (negative)
# tuples should consist of (embedded sentence, embedded function)
# we need CodeBERT and SciBERT model for this

# SciBERT:
# load scibert model like in scibert.py
# input data is in sentence form
# group into dict (var: [sentence])
# 1. tokenize
# 2. embedding from model

# CodeBERT:
# load codexglue model
# import data (.json?) into dict (var: [tokenized function])

# extract a list of all occuring variables
# for each variable, create a pair for each combination of (e_S(sentence), e_C(function)) with label 1
# for class balance, create an equal amount of random combinations with label 0
...
tex_sentences = process_tex_file(f_path, filename)
sdict = populate_sentence_dict(tex_sentences)


#%%
# SciBERT tokenizer/model
...
#%%
# match variables from code and text
# encode
...
for var in all_vars_in_code:
    if var in sdict:
        ...
        n_pairs += len(sdict[var])
        for sk, sv in sdict.items():
            if sk in vdict:
                for i in range(len(sv)):
                    n_pairs += 1
                    pos_samples.append((sdict[sk][i], vdict[sk][0],"1"))

print(f"Number of positive samples created: {n_pairs}")
...
#%%
# now loop through all folders and create samples
...
#%%
t1, c1, l1 = samples[0]
t1_tok = text_tokenizer(t1, return_tensors="pt", padding=True, truncation=True)
t1_emb = text_model(**t1_tok)
c1_tok = code_tokenizer(c1, return_tensors="pt", padding=True, truncation=True)
c1_emb = code_model(**c1_tok)
#TODO: c1 embeddings are fucked maybe?
print(t1_emb, c1_emb, l1)
